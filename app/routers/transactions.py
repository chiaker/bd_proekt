...
async def get_transfers(db: Session = Depends(get_db)):
    transfers = db.query(TransactionBA).all()
    return transfers
...
async def get_transfer(transfer_id: int, db: Session = Depends(get_db)):
    db_transfer = db.query(TransactionBA).filter(TransactionBA.id == transfer_id).first()
    if not db_transfer:
...
async def create_transfer(transfer: TransactionBACreate, db: Session = Depends(get_db)):
    ...
    a_from = db.query(Account).filter(Account.id == transfer.account_id_from).with_for_update().first()
    a_to = db.query(Account).filter(Account.id == transfer.account_id_to).with_for_update().first()
...
async def update_transfer(transfer_id: int, transfer: TransactionBACreate, db: Session = Depends(get_db)):
    db_transfer = db.query(TransactionBA).filter(TransactionBA.id == transfer_id).first()
    if not db_transfer:
        raise HTTPException(status_code=404, detail="Transfer not found")
    if transfer.account_id_from == transfer.account_id_to:
    ...
    # lock accounts
    a_old_from = db.query(Account).filter(Account.id == db_transfer.account_id_from).with_for_update().first()
    a_old_to = db.query(Account).filter(Account.id == db_transfer.account_id_to).with_for_update().first()
    a_new_from = db.query(Account).filter(Account.id == transfer.account_id_from).with_for_update().first()
    a_new_to = db.query(Account).filter(Account.id == transfer.account_id_to).with_for_update().first()
    ...
    if b_from_final < 0:
        raise HTTPException(status_code=400, detail="Insufficient funds in source account after update")
...
async def delete_transfer(transfer_id: int, db: Session = Depends(get_db)):
    ...
    if not db_transfer:
        raise HTTPException(status_code=404, detail="Transfer not found")
    # reverse balances
    a_from = db.query(Account).filter(Account.id == db_transfer.account_id_from).with_for_update().first()
    a_to = db.query(Account).filter(Account.id == db_transfer.account_id_to).with_for_update().first()
    if a_from and a_to:
        amt = Decimal(str(db_transfer.amount))
        if Decimal(a_to.balance) - amt < 0:
            raise HTTPException(status_code=400, detail="Cannot delete transfer: would cause negative balance on destination account")
...
async def create_transaction(transaction: TransactionCreate, db: Session = Depends(get_db)):
    ...
    acc = db.query(Account).filter(Account.id == transaction.account_id).with_for_update().first()
    ...
    if cat.type == 'expense':
        ...
        for b in budgets:
            ...
            if new_spent > Decimal(str(b.amount_limit)):
                raise HTTPException(status_code=400, detail=f"Budget exceeded for category during period {b.period_start} - {b.period_end}")
...
async def update_transaction(transaction_id: int, transaction: TransactionCreate, db: Session = Depends(get_db)):
    ...
    old_acc = db.query(Account).filter(Account.id == db_transaction.account_id).with_for_update().first()
    old_cat = db.query(Category).filter(Category.id == db_transaction.category_id).first()
    new_acc = db.query(Account).filter(Account.id == transaction.account_id).with_for_update().first()
    new_cat = db.query(Category).filter(Category.id == transaction.category_id).first()
    ...
    if old_acc.id == new_acc.id:
        ...
        if Decimal(old_acc.balance) - old_delta < 0:
            raise HTTPException(status_code=400, detail="Insufficient funds on old account for reversal")
        if Decimal(new_acc.balance) + new_delta < 0:
            raise HTTPException(status_code=400, detail="Insufficient funds on new account")
...
async def delete_transaction(transaction_id: int, db: Session = Depends(get_db)):
    ...
    acc = db.query(Account).filter(Account.id == db_transaction.account_id).with_for_update().first()
    cat = db.query(Category).filter(Category.id == db_transaction.category_id).first()
    if acc and cat:
        amt = Decimal(str(db_transaction.amount))
        delta = amt if cat.type == 'income' else -amt
        # ensure deletion won't make balance negative
        if Decimal(acc.balance) - delta < 0:
            raise HTTPException(status_code=400, detail="Cannot delete transaction: would cause negative balance on account")
